...
describe Bringit::Tree, seed_helper: true do
  context :repo do
    let(:repository) { Bringit::Repository.new(TEST_REPO_PATH) }
    let(:tree) { Bringit::Tree.where(repository, SeedRepo::Commit::ID) }
    ...
    describe :dir do
      ...
      it { expect(dir).to be_kind_of Bringit::Tree }
      ...
      context :subdir do
        let(:subdir) { Bringit::Tree.where(repository, SeedRepo::Commit::ID, 'files').first }

        it { expect(subdir).to be_kind_of Bringit::Tree }
      ...
      context :subdir_file do
        let(:subdir_file) { Bringit::Tree.where(repository, SeedRepo::Commit::ID, 'files/ruby').first }

        it { expect(subdir_file).to be_kind_of Bringit::Tree }
    ...
    describe :file do
      ...
      it { expect(file).to be_kind_of Bringit::Tree }
    ...
    describe :submodule do
      ...
      it { expect(submodule).to be_kind_of Bringit::Tree }
