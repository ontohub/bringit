...
describe Bringit::Compare, seed_helper: true do
  ...
  let(:compare) { Bringit::Compare.new(repository, SeedRepo::BigCommit::ID, SeedRepo::Commit::ID, false) }
  let(:compare_straight) { Bringit::Compare.new(repository, SeedRepo::BigCommit::ID, SeedRepo::Commit::ID, true) }
  ...
  describe :commits do
    ...
    context 'non-existing base ref' do
      let(:compare) { Bringit::Compare.new(repository, 'no-such-branch', SeedRepo::Commit::ID) }

      it { is_expected.to be_empty }
    end

    context 'non-existing head ref' do
      let(:compare) { Bringit::Compare.new(repository, SeedRepo::BigCommit::ID, '1234567890') }

      it { is_expected.to be_empty }
    end

    context 'base ref is equal to head ref' do
      let(:compare) { Bringit::Compare.new(repository, SeedRepo::BigCommit::ID, SeedRepo::BigCommit::ID) }

      it { is_expected.to be_empty }
    end

    context 'providing nil as base ref or head ref' do
      let(:compare) { Bringit::Compare.new(repository, nil, nil) }
